...
def _controller(trainer, plan):
    ...
    controller.config = parse_scheduling_config({"enable": True})
...
class TestTrainerWiring:
    ...
    def test_prepare_injects_graph_derived_home_slots(self, monkeypatch):
        ...
        controller = DynamicInferenceController.maybe_create(trainer)
        assert controller is not None
        controller.prepare_runtime_layout()
    ...
    def test_enabled_config_builds_and_setups_controller(self, monkeypatch):
        ...
        class FakeController:
            def __init__(self, config, trainer):
                events.append(("create", config.enable))
            ...
            def prepare_runtime_layout(self):
                events.append(("prepare_runtime_layout",))
        ...
        trainer.policy_trainers = {
            "a": SimpleNamespace(
                init_training_runtime=lambda: events.append(("init_training_runtime",)),
                init_standalone_rollout_runtime=lambda: events.append(
                    ("init_standalone_rollout_runtime",)
                ),
                on_init_end=lambda: events.append(("on_init_end",)),
            ),
        }
        ...
        assert events[:11] == [
            ...
            ("validate_resources", "training"),
            ("init_training_runtime",),
            ("build_dataloader",),
            ("build_replay_buffer",),
            ("load_checkpoint",),
            ("validate_resources", "rollout"),
            ("init_standalone_rollout_runtime",),
            ("setup",),
            ("on_init_end",),
        ]
