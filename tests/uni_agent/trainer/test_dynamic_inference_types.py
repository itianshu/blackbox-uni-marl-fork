...
class TestParseSchedulingConfig:
    ...
    def test_absent_or_disabled_returns_none(self):
        ...
        assert parse_scheduling_config({"enable": False}) is None
    ...
    @pytest.mark.parametrize(
        "borrowing",
        [
            {"guest_master_port_stride": 0},
            {"guest_master_port_stride": 1.5},
            {"guest_master_port_range": [32000]},
            {"guest_master_port_range": [32000, "32768"]},
            {"guest_master_port_range": [1023, 2000]},
            {"guest_master_port_range": [32000, 32004], "guest_master_port_stride": 8},
        ],
    )
    def test_invalid_guest_master_port_pool_rejected(self, borrowing):
        with pytest.raises(ValueError, match="guest_master_port"):
            parse_scheduling_config(_cfg(borrowing=borrowing))

    def test_guest_master_port_pool_is_parsed(self):
        cfg = parse_scheduling_config(_cfg(borrowing={
            "guest_master_port_range": [32000, 32768],
            "guest_master_port_stride": 8,
        }))
        assert cfg.borrowing.guest_master_port_range == [32000, 32768]
        assert cfg.borrowing.guest_master_port_stride == 8
