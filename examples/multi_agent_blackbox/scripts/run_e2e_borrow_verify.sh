#!/usr/bin/env bash
# Single-node 8-GPU E2E that forces policy_2 to lend one rollout replica to
# policy_1, then verifies that training can continue after the swap.
...
export EXCLUDED_PYTHONPATH_ENTRY="${EXCLUDED_PYTHONPATH_ENTRY:-/opt/tiger/bi_verl}"
