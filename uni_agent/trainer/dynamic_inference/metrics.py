...
class VLLMMetricsScraper:
    ...
    def scrape(self, server_address: str) -> VLLMMetricsSnapshot | None:
        ...
        try:
            # Rollout servers are on the cluster's private network.  The
            # runtime sets HTTP(S)_PROXY and Python's proxy bypass helper does
            # not reliably interpret the CIDR-style NO_PROXY entries used by
            # this environment (e.g. ``10.0.0.0/8``).  A direct opener avoids
            # sending internal metrics requests through the external proxy.
