...
class BorrowingConfig:
    ...
    # Optional end-exclusive port pool reserved by the cluster operator for
    # guest torch rendezvous.  None keeps verl's native free-port selection;
    # when set, GuestEngineManager assigns disjoint subranges per master node;
...
def parse_scheduling_config(cfg: Any) -> SchedulingConfig | None:
    ...
    statically invalid combinations (pair folding, ...).
    """
    ...
    guest_port_range = config.borrowing.guest_master_port_range
    ...
    if guest_port_range is not None:
        if (
            ...
            or len(guest_port_range) != 2
            or any(not _is_int(port) for port in guest_port_range)
        ):
            raise ValueError(
                "borrowing.guest_master_port_range must be [start, end] integers"
            )
        start, end = guest_port_range
        if not 1024 <= start < end <= 65536:
            raise ValueError(
                "borrowing.guest_master_port_range must satisfy "
                "1024 <= start < end <= 65536"
            )
        if end - start < guest_port_stride:
            raise ValueError(
                "borrowing.guest_master_port_range must contain at least one "
                "guest_master_port_stride-sized subrange"
            )
...
def validate_policy_prerequisites(
    ...
    for name, rollout_cfg in policy_rollout_configs.items():
        ...
        if _get(rollout_cfg, "disable_log_stats", True) is not False:
            problems.append(
                f"policy '{name}': rollout.disable_log_stats must be false "
                "(dynamic scheduling consumes vLLM /metrics)"
            )
