...
class _GatewayActor:
    def __init__(
        ...
        *,
        ...
        policy_tokenizers: dict[str, Any] | None = None,
        policy_processors: dict[str, Any] | None = None,
        policy_tool_parser_names: dict[str, str] | None = None,
        ...
        self._policy_tokenizers = dict(policy_tokenizers or {})
        self._policy_processors = dict(policy_processors or {})
        self._policy_tool_parser_names = dict(policy_tool_parser_names or {})
        ...
        self._policy_system_prompts = {
            policy_name: initialize_system_prompt(policy_tokenizer, **self._apply_chat_template_kwargs)
            for policy_name, policy_tokenizer in self._policy_tokenizers.items()
        }
        self._policy_tool_parsers = {
            policy_name: ToolParser.get_tool_parser(
                self._policy_tool_parser_names.get(policy_name, tool_parser_name),
                policy_tokenizer,
            )
            for policy_name, policy_tokenizer in self._policy_tokenizers.items()
            if self._policy_tool_parser_names.get(policy_name, tool_parser_name)
        }
        self._sessions: dict[str, GatewaySessionState] = {}
        self._rollouts: dict[str, MultiAgentRolloutState] = {}
        self._app = FastAPI()
    ...
    def _register_routes(self) -> None:
        ...
        @self._app.post("/rollouts/{rollout_id}/v1/chat/completions")
        async def _multi_agent_chat_completions(rollout_id: str, request: Request):
            payload = await request.json()
            return await self._handle_multi_agent_chat_completions(rollout_id=rollout_id, payload=payload)
    ...
    def _get_rollout(self, rollout_id: str) -> MultiAgentRolloutState:
        rollout = self._rollouts.get(rollout_id)
        if rollout is None:
            raise KeyError(f"Unknown rollout_id: {rollout_id}")
        return rollout

    def _resolve_rollout_role(self, rollout_id: str, payload: dict[str, Any]) -> tuple[MultiAgentRolloutState, RoleSessionInfo]:
        rollout = self._rollouts.get(rollout_id)
        if rollout is None:
            raise HTTPException(status_code=404, detail=f"Unknown rollout_id: {rollout_id}")
        model = payload.get("model")
        if not isinstance(model, str) or not model:
            # For multi-agent requests, `model` names the role, not the trainable policy.
            raise HTTPException(status_code=400, detail="Multi-agent rollout requests must set model to a role name")
        role_session = rollout.handle.sessions.get(model)
        if role_session is None:
            raise HTTPException(status_code=404, detail=f"Unknown role {model!r} for rollout_id: {rollout_id}")
        return rollout, role_session
    ...
    async def _extract_multi_modal_data(
        ...
        *,
        ...
        processor = self._processor_for_policy(policy_name)
    ...
    def _tokenizer_for_policy(self, policy_name: str | None):
        if policy_name is None:
            return self._tokenizer
        return self._policy_tokenizers.get(policy_name, self._tokenizer)
    ...
    def _system_prompt_for_policy(self, policy_name: str | None):
        if policy_name is None:
            return self._system_prompt
        return self._policy_system_prompts.get(policy_name, self._system_prompt)

    def _tool_parser_for_policy(self, policy_name: str | None):
        if policy_name is None:
            return self._tool_parser
        return self._policy_tool_parsers.get(policy_name, self._tool_parser)
    ...
    def _encode_full(
        ...
        tokenizer = self._tokenizer_for_policy(policy_name)
        processor = self._processor_for_policy(policy_name)
    ...
    def _encode_incremental(
        ...
        return ids[len(self._system_prompt_for_policy(policy_name)):]

    async def _decode_response(
        self, response_ids: list[int], *, tools: list[dict[str, Any]] | None = None,
        stop_reason: str | None = None, policy_name: str | None = None,
        ...
        """
        tokenizer = self._tokenizer_for_policy(policy_name)
        tool_parser = self._tool_parser_for_policy(policy_name)
        if tool_parser is not None and tools:
            parsed_tools = None
            try:
                ...
                pass
            content, function_calls = await tool_parser.extract_tool_calls(response_ids, parsed_tools)
            if function_calls:
                tool_calls = [
                    {
    ...
    async def _handle_multi_agent_chat_completions(self, rollout_id: str, payload: dict[str, Any]) -> JSONResponse:
        rollout, role_session = self._resolve_rollout_role(rollout_id, payload)
        completion = await self._chat_completions_turn_with_routing(
            session_id=role_session.session_id,
            payload=payload,
            request_id=role_session.session_id,
            routing_metadata={
                "rollout_id": rollout.handle.rollout_id,
                "role": role_session.role,
                "policy_name": role_session.policy_name,
            },
        )
        return JSONResponse(completion)

    async def _chat_completions_turn(self, session_id: str, payload: dict[str, Any]) -> dict[str, Any]:
        return await self._chat_completions_turn_with_routing(session_id=session_id, payload=payload)

    async def _chat_completions_turn_with_routing(
        self,
        session_id: str,
        payload: dict[str, Any],
        *,
        ...
        async with session.generation_lock:
            ...
            async with session.request_lock:
                ...
                policy_name = (routing_metadata or {}).get("policy_name")
                materialized_trajectory = None
                image_data = None
                video_data = None

                if session.active_trajectory is None:
                    image_data, video_data = await self._extract_multi_modal_data(messages, policy_name=policy_name)
                    ...
                    if incremental_messages:
                        new_image_data, new_video_data = await self._extract_multi_modal_data(
    ...
    async def create_multi_agent_rollout(
        self,
        rollout_id: str,
        *,
        ...
        if rollout_id in self._rollouts:
            raise RuntimeError(f"Multi-agent rollout {rollout_id} already exists")
        if not role_policy_mapping:
            raise ValueError("role_policy_mapping must not be empty")

        sessions: dict[str, RoleSessionInfo] = {}
        for role, policy_name in role_policy_mapping.items():
            if not role or not policy_name:
                raise ValueError("role_policy_mapping keys and values must be non-empty strings")
            session_id = f"{rollout_id}:{role}"
            if session_id in self._sessions:
                raise RuntimeError(f"Session {session_id} already exists")
            handle = SessionHandle(session_id=session_id, base_url=f"{self._server_base_url}/sessions/{session_id}/v1")
            session_metadata = {
            ...
            # Each role gets its own Gateway session under the rollout-scoped namespace.
            self._sessions[session_id] = GatewaySessionState(handle=handle, metadata=session_metadata)
            sessions[role] = RoleSessionInfo(role=role, session_id=session_id, policy_name=policy_name)

        handle = MultiAgentRolloutHandle(
            rollout_id=rollout_id,
            base_url=f"{self._server_base_url}/rollouts/{rollout_id}/v1",
            sessions=sessions,
            role_policy_mapping=dict(role_policy_mapping),
        )
        self._rollouts[rollout_id] = MultiAgentRolloutState(handle=handle, metadata=dict(metadata or {}))
        return handle

    async def finalize_multi_agent_rollout(self, rollout_id: str) -> MultiAgentRolloutResult:
        rollout = self._get_rollout(rollout_id)
        async with rollout.request_lock:
            if rollout.phase == SessionPhase.ABORTED:
                raise RuntimeError(f"Multi-agent rollout {rollout_id} is aborted")
            if rollout.phase == SessionPhase.FINALIZED:
                raise RuntimeError(f"Multi-agent rollout {rollout_id} is finalized")

            trajectories: list[Trajectory] = []
            for role, role_session in rollout.handle.sessions.items():
                session = self._sessions.get(role_session.session_id)
                if session is None:
                    continue
                async with session.request_lock:
                    if session.phase == SessionPhase.ABORTED:
                    ...
                    for trajectory in session.trajectories:
                        trajectories.append(
                            replace(
                                trajectory,
                                reward_info=dict(rollout.reward_info),
                                extra_fields={
                    ...
                    self._sessions.pop(role_session.session_id, None)

            rollout.phase = SessionPhase.FINALIZED
            rollout.updated_at = time.time()
            rollout.completed.set()
            self._rollouts.pop(rollout_id, None)
            return MultiAgentRolloutResult(
                rollout_id=rollout_id,
                trajectories=trajectories,
                sessions=dict(rollout.handle.sessions),
                reward_info=dict(rollout.reward_info),
            )
    ...
    async def complete_multi_agent_rollout(
        ...
        rollout = self._get_rollout(rollout_id)
        async with rollout.request_lock:
            if rollout.phase not in {SessionPhase.ACTIVE, SessionPhase.COMPLETED}:
                raise RuntimeError(f"Multi-agent rollout {rollout_id} is {rollout.phase.value.lower()}")
            if reward_info is not None:
                rollout.reward_info = dict(reward_info)
            rollout.phase = SessionPhase.COMPLETED
            rollout.updated_at = time.time()
            rollout.completed.set()

    async def abort_multi_agent_rollout(self, rollout_id: str) -> None:
        rollout = self._rollouts.get(rollout_id)
        if rollout is None:
            return
        async with rollout.request_lock:
            if rollout.phase == SessionPhase.FINALIZED:
                raise RuntimeError(f"Multi-agent rollout {rollout_id} is finalized")
            rollout.phase = SessionPhase.ABORTED
            rollout.updated_at = time.time()
            rollout.completed.set()
            for role_session in rollout.handle.sessions.values():
                session = self._sessions.get(role_session.session_id)
    ...
    async def wait_for_multi_agent_rollout_completion(self, rollout_id: str, timeout: float | None = None) -> None:
        rollout = self._rollouts.get(rollout_id)
        if rollout is None:
            return
        if rollout.phase == SessionPhase.COMPLETED:
            return

        await asyncio.wait_for(rollout.completed.wait(), timeout=timeout)
